...
class GoLangActionContainerTests
    extends BasicActionRunnerTests
    with WskActorSystem {

  lazy val golangContainerImageName = "actionloop-golang-v1.10"

  override def withActionContainer(env: Map[String, String] = Map.empty)(
      code: ActionContainer => Unit) = {
    withContainer(golangContainerImageName, env)(code)
  }

  def withNodeJsContainer(code: ActionContainer => Unit) =
    withActionContainer()(code)

  behavior of golangContainerImageName
  ...
  it should "support the documentation examples (1)" in {
    val (out, err) = withNodeJsContainer { c =>
      ...
      r3.get.fields.get("error") shouldBe Some(
        JsString("payload must be 0 or 1"))
    }
  ...
  it should "support zip-encoded npm package actions" in {
    ...
    val (out, err) = withNodeJsContainer { c =>
      ...
      runRes.get.fields.get("greeting") shouldBe Some(
        JsString("Hello stranger, from an npm package action."))
    }
  ...
  it should "support zip-encoded npm package actions without a package.json file" in {
    ...
    val (out, err) = withNodeJsContainer { c =>
      ...
      runRes.get.fields.get("greeting") shouldBe Some(
        JsString(
          "Hello stranger, from an npm package action without a package.json."))
    }
  ...
  it should "fail gracefully on valid zip files that are not actions" in {
    ...
    checkStreams(
      ...
      err, {
        case (o, e) =>
          (o + e).toLowerCase should include("error")
          (o + e).toLowerCase should include(
